            ...
            <div class="card-body">
                <div class="border rounded-4  p-4 mb-4">
                    ...
                    <form action="remark.php" class="form-horizontal">
                        ...
                        <button type="button" id="add" class="btn btn-warning px-3 mt-4">+ Add More</button>
                        <button  type="button" class="delete btn btn-danger px-3 mt-4">- Remove</button>
...
<script>
    $(document).ready(function() {
        $(".delete").hide();
        var maxRows = 5; // Set the maximum number of rows

        // when the Add Field button is clicked
        $("#add").click(function(e) {
            if ($("#items .next-referral").length < maxRows) {
                 $(".delete").fadeIn("1500");
                // Append a new row of code to the "#items" div
                $("#items").append(
                    `<div class="next-referral col-12  mt-4">
                        <div class="form-group">
                            <label>Title</label>
                            <input type="text" class="form-control" name="name" placeholder="Enter Title">
                        </div>
                ...
                alert("You have reached the maximum limit of " + maxRows + " rows.");
            }
        });

        $("body").on("click", ".delete", function(e) {
            $(".next-referral").last().remove();
            if ($("#items .next-referral").length <= 0) {
                $(".delete").fadeOut("1500");
            }
        });
    });
